...
let ChatID;
let proxyhosts = [];
let myhostsURL = atob(atob('YUhSMGNITTZMeTl5WVhjdVoybDBhSFZpZFhObGNtTnZiblJsYm5RdVkyOXRMMk50YkdsMUwyTnRiR2wxTDIxaGFXNHZVSEp2ZUhsSVQxTlU='));
let 请求CF反代IP = 'false';
...
async function 维列斯OverWSHandler(request) {
    ...
    // @ts-ignore
    const abcd = new WebSocketPair();
    const [client, webSocket] = Object.values(abcd);
...
async function 生成配置信息(userID, hostName, sub, UA, 请求CF反代IP, _url, fakeUserID, fakeHostName, env) {
    ...
    let proxyhost = "";
    if (hostName.includes(".workers.dev")) {
        if (myhostsURL && (!proxyhosts || proxyhosts.length == 0)) {
            try {
                const response = await fetch(myhostsURL);
